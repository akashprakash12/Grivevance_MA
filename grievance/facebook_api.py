...
class FacebookAPI:
    ...
    def post_to_page(self, message=None, image_path=None):
        ...
        if response and 'id' in response:
            logger.info(f"Successfully posted to: Post ID={response['id']}")
        return response
    ...
    def get_comments(self, post_id, time_limit=None):
        ...
        comments = []
        end_time = datetime.now(timezone.utc) + timedelta(hours=time_limit) if time_limit else None
    ...
    def write_comments_to_excel(self, posts=None, comments_dict=None, post_id=None, comments=None):
        if posts and comments_dict:
            columns = [
        ...
        elif post_id and comments:
            if not os.path.exists(os.path.dirname(self.excel_file)):
                os.makedirs(os.path.dirname(self.excel_file))
            ...
            wb.save(self.excel_file)
            logger.info(f"Excel file updated for post {post_id} (grievance format)")